...
export class LocalFfmpegExportService {
  ...
  async export(projectSlug: string, videoPath: string, subtitlePath: string): Promise<string> {
    ...
    await this.ffmpeg.run([
      '-y',
      '-i', this.storage.getAbsolutePath(`${projectPath}/${videoPath}`),
      '-vf', `subtitles=${this.storage.getAbsolutePath(`${projectPath}/${subtitlePath}`)}:force_style='FontName=DejaVu Sans,FontSize=20,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=2,Alignment=2,MarginV=90'`,
      '-c:v', 'libx264',
      '-pix_fmt', 'yuv420p',
