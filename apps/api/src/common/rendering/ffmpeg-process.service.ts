...
export class FfmpegProcessService {
  ...
  async run(args: string[], operation: string): Promise<void> {
    await new Promise<void>((resolve, reject) => {
      ...
      process.on('close', (code: number | null) => code === 0 ? resolve() : reject(new Error(errorOutput)));
    }).catch((error: unknown) => {
      this.logger.error(`FFmpeg failed while attempting ${operation}.`, error instanceof Error ? error.stack : undefined);
