...
export class ProjectsController {
  ...
  @Post(':slug/compose')
  composeVideo(@Param('slug') slug: string) {
    return this.projectsService.composeVideo(slug);
  }

  @Get(':slug/final/download')
  async downloadFinalVideo(@Param('slug') slug: string, @Res() res: Response) {
    const { buffer, filename, contentType } =
      await this.projectsService.downloadFinalVideo(slug);
