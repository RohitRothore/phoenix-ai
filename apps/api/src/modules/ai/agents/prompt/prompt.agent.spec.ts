...
describe('PromptAgent', () => {
  ...
  it('returns validated prompts for every requested scene', async () => {
    const agent = createAgent(
      '{"scenes":[{"id":1,"prompt":"Animated office scene","negativePrompt":"No text","camera":"Medium shot","lighting":"Warm","mood":"Playful"}]}',
    );
  ...
  it('rejects a response with a missing scene', async () => {
    ...
    await expect(agent.execute(input)).rejects.toBeInstanceOf(
